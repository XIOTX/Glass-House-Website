...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const baseLogo = document.querySelector('.logo-base');
    const section = document.querySelector('.logo-reveal-section');

    let currentStep = 0;
    const totalSteps = 5; // 5 cumulative animation steps
    let isTransitioning = false;

    function updateLogoState(step) {
      if (isTransitioning) return;

      currentStep = Math.max(0, Math.min(step, totalSteps));

      // Reset all sections
      logoSections.forEach(section => section.classList.remove('active'));
      baseLogo.classList.remove('final-reveal');

      // Activate cumulative sections based on step
      const sectionOrder = ['top', 'top-right', 'top-right-left', 'top-right-left-in', 'full'];

      // Show only the current step section (cumulative)
      if (currentStep > 0 && currentStep <= sectionOrder.length) {
        const sectionName = sectionOrder[currentStep - 1];
        const sectionEl = document.querySelector(`[data-section="${sectionName}"]`);
        if (sectionEl) sectionEl.classList.add('active');
      }

      // Final reveal on last step (full logo active)
      if (currentStep >= totalSteps) {
        if (!animationComplete) {
          isTransitioning = true;

          // After final reveal, transition to main site
          setTimeout(() => {
    ...
    // More granular scroll handling for slower progression
    function handleScroll() {
      ...
      // Slower progression - each step needs more scroll
      const newStep = Math.floor(scrollProgress * totalSteps);

      if (newStep !== currentStep) {
        updateLogoState(newStep);
      }
    }
  ...
  return (
    <div className="min-h-screen">
      ...
      <style jsx>{`
        ...
        .logo-layer.active {
          opacity: 1;
          filter: blur(0px) brightness(1.2) drop-shadow(0 0 35px rgba(255, 255, 255, 0.7));
        }
